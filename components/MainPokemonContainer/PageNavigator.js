...
import { PageNavigatorContainer, PrevPageArrow, NextPageArrow, PageNumber } from "./Styles"
import { useEffect } from "react";

const getPagesToShow = (currentPage, pokemonPerPage, numberOfMatchedPokemon, maxPages) => {
  let pages = []
  if (currentPage <= 6) {
    ...
    for (let i = currentPage - 4; i <= Math.min(maxPages, currentPage + 4); i++) {
      pages.push(i)
    }
  }
  return pages
...
const PageNavigator = () => {
  ...
  const nextPageExists = (Math.ceil(numberOfMatchedPokemon / pokemonPerPage) > currentPage)

  const pagesToShow = getPagesToShow(currentPage, pokemonPerPage, numberOfMatchedPokemon, maxPages)
  ...
  return (
    <PageNavigatorContainer>
      ...
      {
        pagesToShow.map((page) => <PageNumber onClick={() => goToPage(page)} isActive={page === currentPage}>{page}</PageNumber>)
      }
      {
        nextPageExists &&
