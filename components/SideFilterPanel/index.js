import { MainContainer, StyledInnerContainer, FilterToggleHeading, Divider } from "./Styles.js"
  ...
  return (
    <MainContainer>
      <StyledInnerContainer>
        <FilterToggleHeading menuVisible={priceMenuVisible} handleClick={() => setPriceMenuVisible(!priceMenuVisible)}>Price</FilterToggleHeading>
        {priceMenuVisible &&
          <NumberRangeContainer filter={{ ...priceFilter }} categoryName="Price" currentRange={priceFilter.currentRange} handleChange={handlePriceChange} />
        }
        <TypeFilterContainer menuVisible={typeMenuVisible} handleClick={() => setTypeMenuVisible(!typeMenuVisible)} />
        <AbilityFilterContainer menuVisible={abilityMenuVisible} handleClick={() => setAbilityMenuVisible(!abilityMenuVisible)} />
        <FilterToggleHeading menuVisible={statsMenuVisible} handleClick={() => setStatsMenuVisible(!statsMenuVisible)}>Stats</FilterToggleHeading>
        {statsMenuVisible && Object.keys(statsFilter).map((key) => (
          <>
            <BoldRegularText>{statsFilter[key].name}</BoldRegularText>
            <NumberRangeContainer key={key} filter={{ ...statsFilter[key] }} categoryName={key} handleChange={handleStatsChange} currentRange={statsFilter[key].currentRange} />
          </>
        ))
        }
      </StyledInnerContainer>
