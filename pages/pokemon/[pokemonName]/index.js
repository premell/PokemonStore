...
const pokemon = ({ pokemon }) => {
  return (
    <PokemonPageContainer pokemon={pokemon} />
  )
}
...
export const getStaticPaths = async () => {
  ...
  return {
    paths: defaultPokemonRefs?.results?.map((pokemon) => ({
      params: {
...
export const getStaticProps = async ({ params }) => {
  const pokemonObject = await fetchData(`${BASE_URL}pokemon/${params.pokemonName}`)
  const types = pokemonObject.types.map((type) => type.type.name)
  const abilities = pokemonObject.abilities.map((ability) => ability.ability.name)
  const stats = pokemonObject.stats.map((stat) => ({ [stat.stat.name]: stat.base_stat }))
