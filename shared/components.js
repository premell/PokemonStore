...
const StyledButton = styled.div`
  ...
  `}
  background:${p => {
    if (p.type === "positive") return css`${p.theme.colors.green_light}`
    else if (p.type === "negative") return css`${p.theme.colors.red_light}`
  }};

  &:hover{
    background:${p => {
    if (p.type === "positive") return css`${p.theme.colors.green_hover}`
    else if (p.type === "negative") return css`${p.theme.colors.red_hover}`
  }};
  }
`

export const Button = ({ handleClick, type, innerText, height = "100px", width = "330px" }) => {
  return (
    <StyledButton onClick={handleClick} height={height} width={width} type={type}> {innerText}</StyledButton >
  )
}
...
export const Links = ({ background_color }) => {
  return (
    <StyledLinksContainer background_color="dark">
      ...
      <Link href="https://www.linkedin.com/in/elmer-lingest%C3%A5l-3571021a8/" passHref={true}>
