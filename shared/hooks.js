...
export const useCart = () => {
  ...
  const addPokemonToCart = (pokemon) => {
    const findPokemon = cart.pokemon.filter((cartPokemon) => cartPokemon.name === pokemon.name)
    const pokemonAlreadyInCart = findPokemon.length !== 0
    if (pokemonAlreadyInCart) return
    else {
      setCart(cart => ({
        pokemon: [...cart.pokemon, pokemon], total: cart.total + pokemon.price
      }))
    }
  }

  const removePokemonFromCart = (pokemon) => {
    const updatedPokemon = cart.pokemon.filter((cartPokemon) => cartPokemon.name !== pokemon.name)
    setCart((cart) => ({ pokemon: updatedPokemon, total: cart.total - pokemon.price }))
  }

  return { getCurrentCart, getCurrentTotal, addPokemonToCart, removePokemonFromCart }
}
...
export const useCartModal = () => {
  ...
  const [showModalWithTimer, setShowModalWithTimer] = useRecoilState(showModalWithTimerAtoms);
  const [showModalTemporarily, setShowModalTemporarily] = useRecoilState(showModalTemporarilyAtoms);
  ...
  return { showWithTimer, hideWithTimer, showTemporarily, hideTemporarly, hideInstantly }
}
